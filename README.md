...
- 🔍 **元数据与歌词** — 自动解析音频文件源内嵌的高清专辑封面与 LRC 歌词，支持手动编辑并写入源文件
- 📝 **歌词同步** — 实时解析与展示 LRC 歌词，逐行高亮滚动
- 🖼️ **专辑封面** — 本地解析与持久化缓存嵌入封面
- 🔁 **播放模式** — 顺序播放 / 单曲循环 / 随机播放
- ⏩ **变速播放** — 0.5x~2.0x 播放速度调节，适合播客/学习场景；预设按钮 + 滑块精细控制
- 😴 **睡前定时** — 10~90 分钟定时停止，支持渐弱音量（最后 N 秒线性降低），可延时
- 🖥️ **全屏歌词沉浸交互** — 极简全屏歌词浏览，支持长歌词自动换行左对齐并平滑滚动居中；顶部返回栏与歌曲信息采用居中胶囊悬浮呈现，底部控制栏轻量居中；支持鼠标静止 3 秒或移出窗口自动隐藏控件与光标，歌词流铺满全屏，无遮挡展示。
...
- 💿 **专辑与艺术家视图** — 高性能的封面网格展现与歌手分组浏览。基于本地提取并持久化缓存音频嵌入封面，对大图进行降采样解码限制，零卡顿加载；歌手头像采用拼贴画及生成式哈希渐变色文字头像，完美实现 100% 离线覆盖与现代化高档美感；采用自适应网格比例与紧凑排版，彻底避免不同窗口宽度下的文字溢出。
- 💡 **播放防熄屏** — 在音乐播放状态下处于全屏播放页或全屏歌词页时，自动保持屏幕常亮不熄屏；暂停或离开对应页面后自动恢复系统默认熄屏休眠策略。
- 🖥️ **响应式歌曲表格（桌面端自适应）** — 针对桌面大窗口进行专门排版重构。窗口宽度 ≥ 720px 时自动启用宽屏桌面表格模式，将标题与歌手拆分为独立分列展示，新增文件大小展示与精致格式徽章（无损 FLAC/WAV 强调色），鼠标悬停（Hover）支持背景高亮及快捷操作栏（下一首播放、加入播放队列、更多选项）；窗口拖窄（< 720px）时平滑降级为紧凑双行模式。
- 🪟 **沉浸式一体化桌面顶栏与全页面安全区适配** — 移除系统原生灰色标题栏，内容直通窗口顶部。原生红黄绿交通灯自然内嵌并完美避让，支持在顶栏背景空白处直接拖拽移动窗口与双击最大化；重构整合散乱的搜索框、视图切换胶囊与功能集群为单行高度仅 36px 的极致现代桌面顶栏；全屏播放页、全屏歌词页及全部二级子页面（歌词编辑器、听歌统计、听歌成就等）均深度适配 macOS 原生交通灯区域，返回/收起按钮优雅避让，防止遮挡与误触。
- 🧊 **悬浮玻璃磨砂迷你控制台** — 列表页底部的迷你播放控制台采用 Glassmorphism 悬浮玻璃磨砂设计，单曲列表、专辑网格和歌手视图直通铺满窗口底部；控制台悬浮于列表之上，伴随柔和立体阴影、微光高亮边框与实时 BackdropFilter 模糊滤镜，列表滑动时从控制台下方流动穿透，空间层次通透；同时底部条目具备智能容差内边距，确保最后一首歌永不被遮挡。
...
└── providers/
    ...
    └── wakelock_provider.dart  # 屏幕常亮控制（播放页与歌词页防熄屏）
```
